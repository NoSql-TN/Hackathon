...
def amelioration(data):
    ...
    if new_score > score:
        if new_tempo > tempo:
            improvement['tempo'] = ("Augmenter le tempo à " + str(new_tempo) +" serait bénéfique à votre score.", new_score)
        else:
            improvement['tempo'] = ("Abaissé le tempo à " + str(new_tempo) +" serait bon pour votre musique.", new_score)
    else:
        if new_tempo > tempo:
            good['tempo'] = ("Augmenter le tempo à " + str(new_tempo)+ " n'est pas utile.", new_score)
        else:
            good['tempo'] = ("Abaissé le tempo à " + str(new_tempo)+" ne serait pas bon pour votre musique.", new_score)
    ...
    new_score = predict_popularity(X)
    if new_score > score:
        improvement['key'] = ("Changez la clé majoritaire de votre chanson pour " + notes[new_key] + " serait bénéfique.", new_score)
    else:
        good['key'] = ("Votre clé majoritaire est bonne, la clé: "+ notes[new_key] + " est moins bonne.", new_score)
    ...
    if new_score > score:
        if new_explicit > explicit:
            improvement['explicit'] = ("Mettre des mots Explicits dans votre musique la rendrait meilleur.", new_score)
        else:
            improvement['explicit'] = ("Vous devriez enlever les mots Explicits de votre chanson.", new_score)
    else:
        if new_explicit > explicit:
            good['explicit'] = ("Vous pouvez garder les mots explicits dans votre musique.", new_score)
        else:
            good['explicit'] = ("Vous pouvez garder votre musique sans mots Explicits.", new_score)
    
    # We will try to modify the duration randomly
    new_duration = max(0,duration + random.randint(-30,30))
    
    data = {"lyrics": lyrics, "genre": genre, "artist_name": artist_name, "track_name": track_name, "tempo": tempo, "cle": key , "explicit" : explicit, "duration" : new_duration*1000}
    X = preprocess_data(data)
    
    new_score = predict_popularity(X)
    if new_score > score:
        if new_duration > duration:
            improvement['duration'] = ("Vous devriez allonger votre musique pour atteindre: " + str(new_duration)+"secondes", new_score)
        else:
            improvement['duration'] = ("Vous devriez raccourcir votre musique pour atteindre: " + str(new_duration)+"secondes", new_score)
    else:
        if new_duration > duration:
            good['duration'] = ("Vous pouvez garder votre musique à cette durée.", new_score)
        else:
            good['duration'] = ("Vous pouvez garder votre musique à cette durée.", new_score)
    ...
    if len(new_track_name) > 1:
        bad_word = track_name.split().pop(random.randint(0,len(track_name.split())-1))
        new_track_name = list_of_words_to_try[random.randint(0,len(list_of_words_to_try)-1)]
    else:
        bad_word = None
        new_track_name = list_of_words_to_try[random.randint(0,len(list_of_words_to_try)-1)]
    ...
    if new_score > score:
        if bad_word:
            improvement['track_name'] = ("Ajouter le mot: " + bad_word + " à la place de " + new_track_name+".", new_score)
        improvement['track_name'] = ("Ajouter le mot: " + new_track_name + " à votre musique.", new_score)
    else:
        if bad_word:
            good['track_name'] = ("Remplacer le mot: " + bad_word + " par " + new_track_name+" n'est pas utile.", new_score) 
        good['track_name'] = ("Ajouter le mot: " + new_track_name + " à votre musique n'est pas utile.", new_score)
    ...
    new_score = predict_popularity(X)
    if new_score > score:
        improvement['genre'] = ("Essayez de changer le genre de votre musique pour: " + new_genre, new_score)
    else:
        good['genre'] = ("Votre genre est bon, le genre: " + new_genre + " est moins bon.", new_score)
    ...
    if len(new_lyrics) > 3:
        bad_word= ""
        for i in range(3):
            bad_word+=lyrics.split().pop(random.randint(0,len(lyrics.split())-1))+", "
        for i in range(3):
            new_lyrics+=words_to_try[random.randint(0,len(words_to_try)-1)]+", "
        bad_word = bad_word[:-2]
    else:
        bad_word = None
        for i in range(3):
    ...
    if new_score > score:
        if bad_word:
            improvement['lyrics'] = ("Vous devriez remplacer les mots: " + bad_word + " par " + new_lyrics+".", new_score)
        improvement['lyrics'] = ("Vous devriez ajouter les mots: " + new_lyrics + " à votre musique.", new_score)
    else:
        if bad_word:
            good['lyrics'] = ("Remplacer les mots: " + bad_word + " par " + new_lyrics+" n'est pas utile.", new_score)
        good['lyrics'] = ("Ajouter les mots: " + new_lyrics + " à votre musique n'est pas utile.", new_score)
