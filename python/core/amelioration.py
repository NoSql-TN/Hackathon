...
def amelioration(data):
    ...
    if new_score > score:
        if new_tempo > tempo:
            improvement['tempo'] = ("Augmenter le tempo à" + str(new_tempo), new_score)
        else:
            improvement['tempo'] = ("Abaissé le tempo à " + str(new_tempo), new_score)
    else:
        if new_tempo > tempo:
    ...
    if len(new_lyrics) > 3:
        bad_word= []
        for i in range(3):
            bad_word.append(lyrics.split().pop(random.randint(0,len(lyrics.split())-1)))
        for i in range(3):
